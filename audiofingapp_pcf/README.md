Audio Fingerprinting App on PCF
==========================================
This repo contains the code required to build an audio fingerprinting app on PCF. The audio fingerprinting framework is built within the Pivotal Greenplum Database.
The app that is hosted on PCF allows users to input an soundcloud URL of a query snippet and then invokes the ffmpeg decoder,audio fingerprint extraction and performs matching against audio fingerprints stored in the Greenplum Database to retrieve matching songs and a time offset at which the match was found. For the modeling pipeline, check out: https://github.com/gautamsm/data-science-on-mpp/tree/master/computer_vision
